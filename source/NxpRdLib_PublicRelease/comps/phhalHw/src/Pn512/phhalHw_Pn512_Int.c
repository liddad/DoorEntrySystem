                                   ...
                                   )
{
    ...
    phStatus_t p = phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_FIFOLEVEL, &bValue);
    PH_CHECK_SUCCESS_FCT(statusTmp, p);
    ...
    )
{
    ...
    /* start sending */
    if ((bCmdCode == PHHAL_HW_PN512_CMD_TRANSCEIVE) ||
    	(pDataParams->bCardType == PHHAL_HW_CARDTYPE_I18092MPT) ||
    	(pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443A_CE))
    {
    ...
    )
{
    ...
    {
    	bIrq0WaitFor = PHHAL_HW_PN512_BIT_RXI | PHHAL_HW_PN512_BIT_ERRI | PHHAL_HW_PN512_BIT_HIALERTI;
        bIrq1WaitFor = PHHAL_HW_PN512_BIT_RFOFFI;

        if(pDataParams->bDontUseIrq)
        {
    		PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_WaitIrq(
    			pDataParams,
    			bIrq0WaitFor,
...
//                while ((!(bIrq0Rq & bIrq0WaitFor))/* && (bStatus2 & 0x07)*/);
        ...
        {
//        	bIrq0WaitFor = PHHAL_HW_PN512_BIT_ERRI;
//        	bIrq1WaitFor = 0x00;
//
//        	do
//        	{
...
//                /* now see if there was an error or modem status went to idle - but mask out FIFO HiAlert */
//        	}
//        	while (((!(bIrq0Rq & bIrq0WaitFor)) && ((bStatus2 & 0x07) == 0x06)) || (wFifoBytes != 0));
            do
            {
                do
                {
                    /* check if there is an error or modem status is not "receiving" */
                    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_COMMIRQ, &bIrq0Rq));
                    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_STATUS2, &bStatus2));
                }
                while ((!(bIrq0Rq & bIrq0WaitFor)) && ((bStatus2 & 0x07) == 0x06));
                ...
                /* now see if there was an error or modem status is not "receiving"  - but mask out FIFO HiAlert */
            }
            while (((!(bIrq0Rq & (bIrq0WaitFor & (~PHHAL_HW_PN512_BIT_HIALERTI)))) && ((bStatus2 & 0x07) == 0x06)) || (wFifoBytes != 0));
        }
